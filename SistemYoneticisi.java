...
public class SistemYoneticisi {
    ...
    // GÜNCELLENDİ: gunlukSimulasyonuBaslat
    public int[] gunlukSimulasyonuBaslat() {
        System.out.println("Yeni gün başlıyor... Önceki günden kalanlar kontrol ediliyor...");
        kalanAraclariCikisYap("Otomatik Gün Sonu Çıkışı (Önceki Gün)");
        this.gunSayaci++;
        int basariliGirisSayisi = 0;
        int basariliCikisSayisi_Simulasyon = 0;
        System.out.println("KULLANICI TETİKLEDİ: GÜN " + this.gunSayaci + " SİMÜLASYONU BAŞLIYOR.");
        
        int girisDenemeSayisi = 50;
        for (int i = 0; i < girisDenemeSayisi; i++) {
            String aracID = String.valueOf(rastgele.nextInt(40) + 1);
            
            // --- BURASI GÜNCELLENDİ ---
            // Artık 'null' yerine rastgele bir giriş yeri yolluyoruz
            if (giseIslemYap(aracID, "GIRIS", getRandomGirisYeri())) {
                basariliGirisSayisi++;
            }
            // --- GÜNCELLEME SONU ---
        }
        
        int cikisDenemeSayisi = 25;
        for (int i = 0; i < cikisDenemeSayisi; i++) {
            if (suAnIcrideOlanAraclar.isEmpty()) {
                break;
            }
            int rastgeleIndex = rastgele.nextInt(suAnIcrideOlanAraclar.size());
            String aracID = suAnIcrideOlanAraclar.get(rastgeleIndex);
            if (giseIslemYap(aracID, "CIKIS", getRandomRotalar())) {
                basariliCikisSayisi_Simulasyon++;
            }
        }
        
        System.out.println("Günlük simülasyon tamamlandı. Gün sonu temizliği yapılıyor...");
        kalanAraclariCikisYap("Otomatik Gün Sonu Çıkışı (Mevcut Gün)");
        System.out.println("Gün " + this.gunSayaci + " tamamlandı. " + "İçeride kalan araç sayısı: " + suAnIcrideOlanAraclar.size());
        return new int[]{basariliGirisSayisi};
    }
    ...
    // YENİ YARDIMCI METOT: Sadece 1 tane rastgele giriş yeri seçer
    private String[] getRandomGirisYeri() {
        String girisYeri = ROTA_LISTESI.get(rastgele.nextInt(ROTA_LISTESI.size()));
        return new String[]{ girisYeri };
    }
